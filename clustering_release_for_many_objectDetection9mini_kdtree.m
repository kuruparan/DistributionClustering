...
function [final_sol,clus_dist]=clustering_release(D, thres, min_num_per_clus, max_dis,mini)
if(~exist('thres', 'var'));
    thres=0.07;
end;
...
%             end;            
            ...
            if(alloc2(k)>0 || not(ismember(k,out_I)))
                continue;
            end;    
%             vec_tmp=cur_vec;
%             vec_tmp(k)=0;
        ...
        %pause(1); 
            for kk =1:length(maskkilist)
                ...
                if alloc2(kk)==0
                    ...
                    [out_K]=kdtreee2(D(kk,:),mmz,kk,slice,20);

                    vec_k_5=vec_k(out_K);
                    vec_k=vec_k_5;    
                    [vec_tmp2,vec_k2,out_I2,out_K2]=organize(out_I,vec_tmp,out_K,vec_k,20);
           %         pause(1);    
                     vec_tmp=vec_tmp2;
                     vec_k=vec_k2;
